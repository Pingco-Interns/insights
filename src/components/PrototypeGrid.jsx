...
function PrototypeGrid(props) {
  ...
  const layoutList = dataProcessed.map((i) => i.layout); //layout defines the visuals for each element.
  ...
  const [addText, setText] = useState("Insert JSON data here. Soon to be revamped to be more user friendly")
  ...
  return (
    <div>
            ...
            <div style={{ border: "1px solid black" }} key={i.key}>
              ...
              <Tooltip title="Show table">
                <Button
                  ...
                  onClick={(e) => {
                    setTableDialogOpen();
                    setCurrentData({ keys: i.keys, data: i.data });
            ...
            <Table className={{ minWidth: 650 }} size="small">
              ...
              <TableBody>
                {currentData.data.map((i) => {
                  ...
                  return (
                    <TableRow>
                      <TableCell align="right">{i.country}</TableCell>
                      {currentData.keys.map((j) => {
                        return <TableCell align="right">{i[j]}</TableCell>;
                      })}
                    </TableRow>
                  );
                })}
      ...
      <Dialog open={showAddDialog} onClose={setAddDialogClose} aria-labelledby="form-dialog">
        <DialogTitle id="form-dialog">Add an entry</DialogTitle>
        <DialogContent>
          <DialogContentText>
              <TextareaAutosize minRows={4} maxRows={10} value={addText} onChange={handleAddTextChange}>
              </TextareaAutosize>
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      <Fab color="primary" onClick={setAddDialogOpen} style={butStyle} aria-label="add">
        <AddIcon />
      </Fab>
    </div>
  );
